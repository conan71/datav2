...
const Config = (props: Props) => {
  ...
  const { screenName, size, box, backgroundColor, backgroundImage, changeScreen } = props;
  ...
  const getConfig = () => {
    if (active.length === 0) {
      return <ScreenConfig screenName={screenName} size={size} backgroundColor={backgroundColor} backgroundImage={backgroundImage} changeScreen={changeScreen} />;
    } else if (data && active.length === 1) {
  ...
  return (
    <>
      ...
      <div className={styles.config} style={{ width: `${width > 300 ? width + 'px' : '300px'}` }}>
        <div className={styles.header}>{active.length == 0 ? screenName : active.length == 1 ? box[active[0]]?.data.title : ''}</div>
